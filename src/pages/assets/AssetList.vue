...
const assets: Asset[] = [
  { id: 'A001', name: 'Laptop Dell XPS', type: 'Physical', status: 'Active', department: 'IT', assignedTo: 'John Doe', purchaseDate: '2025-12-01' },
  { id: 'A002', name: 'Adobe CC License', type: 'Soft', status: 'Assigned', department: 'Design', assignedTo: 'Jane Smith', purchaseDate: '2025-11-10' },
  { id: 'A003', name: 'Projector Epson', type: 'Physical', status: 'Maintenance', department: 'HR', assignedTo: 'N/A', purchaseDate: '2024-09-15' },
];
...
<template>
  <div class="h-full flex flex-col gap-6">
    ...
    <div class="flex justify-between items-center">
      ...
      <Button label="Add New Asset" type="primary" @click="router.push({ name: 'asset-new' })" />
    </div>
    ...
    <div class="flex-1 overflow-auto bg-app-surface rounded-lg border border-app-border-pill">
      <Table :headers="['ID', 'Name', 'Type', 'Status', 'Department', 'Assigned To', 'Purchase Date', 'Actions']">
        <template v-for="asset in assets" :key="asset.id">
          ...
          >
            ...
            <td class="px-4 py-3 text-sm font-bold">
              <Badge
                :text="asset.status"
                :color="asset.status === 'Active' ? 'success' : asset.status === 'Assigned' ? 'brand' : asset.status === 'Maintenance' ? 'warning' : 'dim'"
              />
            </td>
            ...
            <td class="px-4 py-3 text-sm flex gap-2">
              ...
              />
              <Button label="Delete" type="danger" small @click.stop="alert('Delete mock!')" />
            </td>
          </tr>
        </template>
      </Table>
